    ...
    <div class="ventas-container">
      <div class="header-section">
        <div class="title-box">
          ...
          <p class="subtitle">Emisión de Facturas, Notas de Crédito y Débito (Anexo 1)</p>
        </div>
        ...
        <div v-if="!mostrandoLista" class="card fade-in">
          ...
          <form @submit.prevent="guardarVenta" class="form-body">
            ...
            <div v-if="formulario.tipoDocumento === '05'" class="alert alert-warning mb-4 fade-in">
                <strong>💡 Llenado de Nota de Crédito:</strong> Ingrese el monto que está <b>devolviendo o descontando</b> de la factura original. <em>(Nota: Ingrese los valores en positivo, Hacienda los restará).</em>
            </div>
            <div v-if="formulario.tipoDocumento === '06'" class="alert alert-info mb-4 fade-in">
                <strong>💡 Llenado de Nota de Débito:</strong> Ingrese el <b>cargo extra o excedente a cobrar</b> que no se incluyó en la Factura original.
            </div>
              ...
              <div class="form-grid four-cols">
                ...
                <div class="form-group fade-in" style="grid-column: span 2;" v-if="formulario.modoIngreso === 'dte'">
                   ...
                   <div class="dte-mask-container">
                      ...
                      <input type="text" :value="dteParts.part1" class="dte-part w-2ch bg-transparent" readonly>
                      <input type="text" :value="dteParts.letraSerie" @input="handleLetraInput" class="dte-part dte-letter" placeholder="S" @focus="$event.target.select()">
                      <input type="text" :value="dteParts.part2" @input="e => handleInputMask(e, 'part2', 3)" class="dte-part w-3ch" placeholder="000">
            ...
            <div class="form-section">
              <h3 class="section-title">👤 Cliente (Opcional)</h3>
              <div class="form-grid">
                <div class="form-group">
              ...
              <div class="montos-wrapper">
                <div class="monto-group">
                    <label class="monto-label">
                        {{ formulario.tipoDocumento === '05' ? 'Devolución / Descuento (Total)' : (formulario.tipoDocumento === '06' ? 'Cargo Extra (Total)' : 'Ventas Gravadas') }}
                    </label>
                    <div class="input-wrapper">
                        <span class="currency text-dark">$</span>
                        <input type="number" v-model="formulario.gravadas" step="0.01" class="form-control monto-input text-dark" @blur="formatearDecimal('gravadas')">
                    </div>
                </div>
                ...
                <div class="monto-group total-group">
                    <label class="monto-label text-primary">
                        {{ formulario.tipoDocumento === '05' ? 'TOTAL NOTA DE CRÉDITO' : (formulario.tipoDocumento === '06' ? 'TOTAL NOTA DE DÉBITO' : 'TOTAL FACTURA') }}
                    </label>
                    <div class="input-wrapper">
                        <span class="currency text-primary">$</span>
                        <input :value="totalCalculado" type="text" class="form-control total-input text-primary" readonly>
                    </div>
                </div>
              </div>
            </div>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="venta in ventasFiltradas" :key="venta.idconsfinal" :class="{'selected-row': seleccionados.includes(venta.idconsfinal), 'is-anulado': esAnulado(venta)}">
                  ...
                  <td>
                     <span class="badge-type" :class="{'blue': venta.ConsTipoDoc === '01', 'orange': venta.ConsTipoDoc === '05', 'purple': venta.ConsTipoDoc === '06'}">
                  ...
                  <td class="text-right fw-bold text-dark">
                    <span v-if="venta.ConsTipoDoc === '05'" class="text-danger">-</span>${{ parseFloat(venta.ConsTotalVta || 0).toFixed(2) }}
                  </td>
                  <td class="text-center">
