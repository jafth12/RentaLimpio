    ...
    <div class="rl-view">
      ...
      <div class="rl-view-header">
        <div class="rl-view-title">
          <h1>🧾 Ventas a Consumidor Final</h1>
          <p class="rl-view-subtitle">Registro de Tickets, Facturas y DTEs a Clientes Finales · Anexo 1</p>
        </div>
      ...
      <div v-if="!mostrandoLista" class="rl-card rl-fade-in">
        <div class="rl-card-header" style="align-items: center;">
          <div>
            <h2>{{ modoEdicion ? '✏️ Editar Registro' : '✨ Nuevo Documento (o Rango de Tickets)' }}</h2>
            <span class="rl-badge rl-badge-info rl-mt-2">
              {{ modoEdicion ? 'Modificando registro existente' : 'Ventas facturadas al Consumidor Final' }}
            </span>
          </div>
        ...
        <form @submit.prevent="guardarVenta">
          ...
          <div v-if="formulario.tipoDocumento === '05' || formulario.tipoDocumento === '06'" class="rl-alert" style="background:#eff6ff;color:#1e40af;border-color:#bfdbfe;margin-bottom:16px">
            <strong>💡 Nota de Ajuste:</strong> Este documento se registrará temporalmente en Anexo 1 y se enviará al reporte de Notas de Crédito/Débito.
          </div>
            ...
            <div class="rl-grid rl-grid-2">
              ...
              <div class="rl-field">
                ...
                <select v-model="formulario.tipoDocumento" class="rl-select" style="font-weight:700" required>
                  <option value="01">01 - Factura / Ticket a Consumidor Final</option>
          ...
          <div v-if="formulario.modoIngreso === 'dte'" class="rl-form-section rl-fade-in">
             <p class="rl-section-title">Comprobante Electrónico (DTE)</p>
             <div class="rl-field">
             ...
             <div class="rl-dte-group rl-mt-3">
                ...
                <div class="rl-field rl-field-sello">
                  ...
                  <span class="rl-sello-hint">Aplica solo para DTE</span>
                </div>
             </div>
          </div>

          <div v-if="formulario.modoIngreso === 'fisico'" class="rl-form-section rl-fade-in">
             <p class="rl-section-title">Documentos Físicos (Rango de Papel / Tickets)</p>
             <div class="rl-grid rl-grid-2">
               <div class="rl-field">
                 <label class="rl-label">Correlativo DEL <span class="req">*</span></label>
                 <input type="text" v-model="formulario.desde" class="rl-input" placeholder="Ej: 100" style="font-weight:700" :required="formulario.modoIngreso === 'fisico'">
               </div>
               <div class="rl-field">
                 <label class="rl-label">Correlativo AL <span class="req">*</span></label>
                 <input type="text" v-model="formulario.hasta" class="rl-input" placeholder="Ej: 150" style="font-weight:700" :required="formulario.modoIngreso === 'fisico'">
               </div>
             </div>
             <div class="rl-grid rl-grid-3 rl-mt-3">
               <div class="rl-field">
                 <label class="rl-label">Resolución <span class="req">*</span></label>
                 <input type="text" v-model="formulario.resolucion" class="rl-input" placeholder="15042-RES..." :required="formulario.modoIngreso === 'fisico'">
               </div>
               <div class="rl-field">
                 <label class="rl-label">Máquina Registradora</label>
                 <input type="text" v-model="formulario.maquina" class="rl-input" placeholder="Opcional para tickets">
               </div>
               <div class="rl-field">
                 <label class="rl-label">Serie</label>
                 <input type="text" v-model="formulario.serie" class="rl-input" placeholder="Opcional">
               </div>
             </div>
          </div>

          <div class="rl-form-section">
            <p class="rl-section-title">Identificación del Cliente</p>
            <div class="rl-grid rl-grid-2">
              <div class="rl-field">
                <label class="rl-label">Nombre del Cliente</label>
                <input type="text" v-model="formulario.cliente" class="rl-input" placeholder="Cliente General">
              </div>
              <div class="rl-field">
                <label class="rl-label">Documento Identidad (DUI / NIT / Pasaporte)</label>
                <input type="text" v-model="formulario.documentoCliente" class="rl-input" placeholder="Opcional">
              </div>
            </div>
          </div>

          <div class="rl-form-section rl-bg-soft">
            <p class="rl-section-title">Clasificación y Totales</p>
            ...
            <div class="rl-montos">
              <div class="rl-monto-item">
                <span class="rl-monto-label">Ventas Gravadas</span>
              ...
              <div class="rl-monto-item is-total">
                <span class="rl-monto-label">TOTAL (IVA INCLUIDO)</span>
                <div class="rl-monto-wrap">
          ...
          <div class="rl-form-actions">
            ...
            <button type="submit" class="rl-btn rl-btn-success rl-btn-lg" :disabled="cargando">
              {{ cargando ? 'Procesando...' : (modoEdicion ? '✔ Actualizar Registro' : '💾 Guardar Ventas') }}
            </button>
          </div>
      ...
      <div v-else class="rl-card rl-fade-in">
        <div class="rl-card-header">
          <div style="display:flex;align-items:center;gap:10px">
            <h3>📋 Historial Consumidor Final (Anexo 1)</h3>
            <span class="rl-badge rl-badge-count">{{ ventasFiltradas.length }} registros</span>
          </div>
          <div class="rl-filters">
            ...
            <input type="text" v-model="filtro" placeholder="🔍 DTE / Doc Físico..." class="rl-input rl-filter-search">
          </div>
        </div>
          ...
          <table class="rl-table">
            <thead>
              <tr>
                ...
                <th>Fecha</th><th>Tipo</th><th>Clase</th><th>Doc. Referencia</th>
                <th style="text-align:right">Total (Gravado)</th>
            ...
            <tbody>
              <tr v-for="venta in ventasFiltradas" :key="venta.idconsfinal"
                ...
                <td>
                  <span class="rl-badge rl-badge-type" :class="venta.ConsTipoDoc === '01' ? 'blue' : 'orange'">
                    {{ venta.ConsTipoDoc === '05' ? 'NC (05)' : venta.ConsTipoDoc === '06' ? 'ND (06)' : 'FAC/Ticket (01)' }}
                  </span>
                </td>
                <td>
                  <span class="rl-badge" :class="venta.ConsClaseDoc === '4' ? 'rl-badge-info' : 'rl-badge-anexo'">
                    {{ venta.ConsClaseDoc === '4' ? 'DTE' : 'Físico' }}
                  </span>
                </td>
                <td>
                  <div v-if="venta.ConsClaseDoc === '4'">
                      <span class="rl-doc-number" :title="venta.ConsCodGeneracion">{{ venta.ConsNumDocAL || 'N/A' }}</span>
                      <div v-if="venta.ConsSelloRecepcion" style="margin-top:3px">
                        <span class="rl-badge-sello-mh" :title="venta.ConsSelloRecepcion">✔ Sello MH</span>
                      </div>
                  </div>
                  <div v-else>
                      <span class="rl-doc-number">Rango: {{ venta.ConsNumDocDEL }} - {{ venta.ConsNumDocAL }}</span>
                  </div>
                </td>
                <td style="text-align:right;font-weight:700;color:#059669">${{ parseFloat(venta.ConsTotalVta || 0).toFixed(2) }}</td>
                <td style="text-align:center">
                  ...
                  <button class="rl-btn-icon" @click="anularDocumento(venta)" title="Anular Rango/Doc" style="color:#d97706">🚫</button>
                </td>
              </tr>
...
const ccfParts = ref({ part1: '01', letraSerie: 'S', part2: '000', part3: '000', part4: '000000000000000' });
    ...
    try {
        const promesas = seleccionados.value.map(id => {
            ...
            const payload = {
                ...
                numero_control: ventaOri.ConsNumDocDEL, // El backend lo absorbe
                uuid_dte: ventaOri.ConsCodGeneracion,
                sello_recepcion: ventaOri.ConsSelloRecepcion,
...
watch(() => [formulario.value.gravadas, formulario.value.exentas, formulario.value.noSujetas], () => {
    ...
    formulario.value.total = (g + e + ns).toFixed(2);
});
...
const ventasFiltradas = computed(() => {
    ...
    if (filtro.value) {
        ...
        filtrado = filtrado.filter(v => (v.ConsNomRazonCliente && v.ConsNomRazonCliente.toLowerCase().includes(txt)) || (v.ConsNumDocAL && v.ConsNumDocAL.toLowerCase().includes(txt)));
    }
    return filtrado;
});
...
const eliminarVenta = async (venta) => { 
    ...
    try { 
        const idBorrar = venta.idconsfinal;
...
const prepararEdicion = (venta) => { 
    ...
    const rawNum = venta.ConsNumDocAL || venta.ConsNumDocDEL || '';
    const esDTE = venta.ConsClaseDoc === '4';
    
    if (esDTE) {
        const regex = /^DTE(\d{2})([A-Z0-9])(\d{3})P(\d{3})(\d{15})$/;
        const match = rawNum.replace(/-/g, '').match(regex);
        ccfParts.value = match ? { part1: match[1], letraSerie: match[2], part2: match[3], part3: match[4], part4: match[5] } : { part1: venta.ConsTipoDoc || '01', letraSerie: 'S', part2: '000', part3: '000', part4: '000000000000000' };
    } else {
        ccfParts.value = { part1: '01', letraSerie: 'S', part2: '000', part3: '000', part4: '000000000000000' };
    }
    ...
    formulario.value = { 
        ...
        sello_recepcion: venta.ConsSelloRecepcion || '',
        desde: esDTE ? '' : venta.ConsNumDocDEL,
        hasta: esDTE ? '' : venta.ConsNumDocAL,
        serie: venta.ConsSerieDoc || '',
        resolucion: venta.ConsNumResolu || '',
        maquina: venta.ConsNumMaqRegistro || '',
        cliente: venta.ConsNomRazonCliente || 'Cliente General',
        documentoCliente: venta.ConsNumDocIdentCliente || '',
        tipo_operacion: limpiarCodigoCat(venta.ConsTipoOpera) || '1',
        tipo_ingreso: limpiarCodigoCat(venta.ConsTipoIngreso) || '1',
...
const resetForm = () => { 
    formulario.value = { 
        ...
        uuid_dte: '', 
        sello_recepcion: '', 
        desde: '', hasta: '', serie: '', resolucion: '', maquina: '',
...
const anularDocumento = async (ventaOriginal) => {
    if(esAnulado(ventaOriginal)) { alert("Este documento ya se encuentra anulado."); return; }
    if(!confirm('⚠️ ¿Está seguro que desea ANULAR este documento o rango?\nSe marcará como inoperante en pantalla y no sumará en los reportes.')) return;
    ...
    try {
        const payloadAnulado = {
            ...
            sello_recepcion: ventaOriginal.ConsSelloRecepcion || '',
            desde: ventaOriginal.ConsNumDocDEL, 
