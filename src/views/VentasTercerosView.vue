    ...
    <div class="terceros-container">
      ...
      <div class="header-section">
        <div class="title-box">
          ...
          <p class="subtitle">Gestión de operaciones y comisiones por mandato (Anexo 4)</p>
        </div>
      ...
      <div class="main-content">
        ...
        <div v-if="!mostrandoLista" class="card fade-in">
          <div class="card-header">
            ...
            <span class="badge-info">{{ modoEdicion ? 'Modificando datos' : 'Ingreso de documento DTE o Físico' }}</span>
          </div>

          <form @submit.prevent="guardarVenta" class="form-body">
            
            <div class="form-section">
              <h3 class="section-title">📄 Datos del Documento Emitido</h3>
            ...
            <div class="form-section">
              <h3 class="section-title">👤 Datos del Mandante (A quien representa)</h3>
              <div class="form-grid">
                <div class="form-group">
                  <label class="form-label">NIT del Mandante <span class="text-danger">*</span></label>
                  <input type="text" v-model="formulario.nitMandante" class="form-control" placeholder="0000-000000-000-0" required>
                </div>
                <div class="form-group">
                  <label class="form-label">Nombre del Mandante <span class="text-danger">*</span></label>
                  <input type="text" v-model="formulario.nombreMandante" class="form-control" placeholder="Nombre completo o Razón Social" required>
                </div>
              </div>
            </div>
            ...
            <div class="form-section bg-light">
              ...
              <div class="montos-wrapper">
                <div class="monto-group">
                  <label class="monto-label">Monto Total de Operación ($)</label>
                  <div class="input-wrapper">
                    <span class="currency">$</span>
                    <input type="number" v-model="formulario.gravadas" step="0.01" class="form-control monto-input" @blur="formatearDecimal('gravadas')" placeholder="0.00">
                  </div>
                </div>
                
                <div class="monto-group">
                  <label class="monto-label text-success">IVA de Operación (13%)</label>
                  <div class="input-wrapper">
                    <span class="currency text-success">$</span>
                    <input type="number" v-model="formulario.comision" step="0.01" class="form-control monto-input text-success" @blur="formatearDecimal('comision')" placeholder="0.00">
                  </div>
                  <small class="text-xs text-muted">Auto-calculado (Puede editarlo)</small>
                </div>
              </div>
            </div>
        ...
        <div v-else class="card fade-in">
          <div class="card-header flex-between flex-wrap gap-3">
             ...
             <div class="history-filters">
                ...
                <input type="text" v-model="filtroBusqueda" placeholder="🔍 Mandante o DTE..." class="form-control search-list">
             </div>
          </div>
            ...
            <table class="table">
              <thead>
                <tr>
                  ...
                  <th class="text-right text-success">IVA Operación</th>
                  <th class="text-center">Acciones</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="venta in ventasFiltradas" :key="venta.idVtaGravTerDomici" :class="{'selected-row': seleccionados.includes(venta.idVtaGravTerDomici)}">
                  <td class="text-center">
                     <input type="checkbox" :value="venta.idVtaGravTerDomici" v-model="seleccionados" class="row-checkbox">
                  </td>
                  <td>
                    <div class="fw-bold text-dark">{{ formatearFecha(venta.VtaGraTerFecha) }}</div>
                    <small class="text-muted">Declarado: <strong class="text-primary">{{ venta.VtaGraTerMesDeclarado || 'N/A' }}</strong></small>
                  </td>
                  <td><span class="badge-anexo">Anexo 4</span></td>
                  <td>
                    <div class="fw-bold text-dark">{{ venta.VtaGraTerNom }}</div>
                    <small class="text-muted">{{ venta.VtaGraTerNit }}</small>
                  </td>
                  <td><span class="doc-number">{{ venta.VtaGraTerNumDoc }}</span></td>
                  <td class="text-right fw-bold text-dark">${{ parseFloat(venta.VtaGraTerMontoOper || 0).toFixed(2) }}</td>
                  <td class="text-right fw-bold text-success">${{ parseFloat(venta.VtaGraTerIVAOper || 0).toFixed(2) }}</td>
                  <td class="text-center">
                    <button class="btn-icon" @click="prepararEdicion(venta)" title="Editar">✏️</button>
                    <button class="btn-icon text-danger" @click="eliminarVenta(venta.idVtaGravTerDomici)" title="Eliminar">🗑️</button>
                  </td>
                </tr>
                <tr v-if="ventasFiltradas.length === 0">
                  <td colspan="8" class="text-center py-4 text-muted">No se encontraron registros de ventas a terceros.</td>
                </tr>
              </tbody>
            </table>
          </div>
...
.text-danger { color: #ef4444; } .text-muted { color: #6b7280; } .text-primary { color: #55C2B7; } .text-success { color: #10b981; }
